...
const CrearUsuarioCsv = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${urlBase}/usuarios/alta-csv`, formData, config);
      ...
      if (totalRegistrados > 0) {
        toast.success(`${totalRegistrados} usuarios registrados exitosamente`);
      }
      if (totalErrores > 0) {
      ...
      {resultado && (
        <ResultadoContainer>
          ...
          {resultado.registrados && resultado.registrados.length > 0 && (
            <ListaExitosa>
              <SubTitulo>✅ Usuarios registrados exitosamente ({resultado.registrados.length}):</SubTitulo>
              {resultado.registrados.map((email, index) => (
...
export default CrearUsuarioCsv;