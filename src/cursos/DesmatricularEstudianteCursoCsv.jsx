...
const DesmatricularEstudianteCursoCsv = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${urlBase}/cursos/desmatricular-estudiantes-csv?idCurso=${cursoActual.id}`, formData, config);
      setResultado(response.data);
      const totalDesmatriculados = response.data.desmatriculados?.length || 0;
      const totalErrores = response.data.errores?.length || 0;
      if (totalDesmatriculados > 0) {
        toast.success(`${totalDesmatriculados} estudiantes desmatriculados exitosamente`);
      }
      if (totalErrores > 0) {
  ...
  return (
    ...
    <ContentWrapper>
      ...
      >
        {loading ? "Procesando..." : "Desmatricular Estudiantes"}
      </UploadButton>
      {resultado && (
        <ResultadoContainer>
          <ResultadoTitulo>Resultados de la carga</ResultadoTitulo>
          {resultado.desmatriculados && resultado.desmatriculados.length > 0 && (
            <ListaExitosa>
              <SubTitulo>✅ Estudiantes desmatriculados exitosamente ({resultado.desmatriculados.length}):</SubTitulo>
              {resultado.desmatriculados.map((id, index) => (
...
export default DesmatricularEstudianteCursoCsv;
