...
const MatricularEstudianteCursoCsv = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.log('Enviando archivo:', selectedFile.name, 'Size:', selectedFile.size);
      const response = await axios.post(`${urlBase}/cursos/matricular-estudiantes-csv?idCurso=${cursoActual.id}`, formData, config);
      
      console.log('Respuesta exitosa:', response.data);
      setResultado(response.data);
      
      const totalMatriculados = response.data.matriculados?.length || 0;
      const totalErrores = response.data.errores?.length || 0;
      
      if (totalMatriculados > 0) {
        toast.success(`${totalMatriculados} estudiantes matriculados exitosamente`);
      }
      if (totalErrores > 0) {
  ...
  return (
    <Container>
    <ContentWrapper>
      <Title>Matricular Estudiantes desde CSV</Title>
      ...
      >
        {loading ? "Procesando..." : "Matricular Estudiantes"}
      </UploadButton>

      {resultado && (
        <ResultadoContainer>
          <ResultadoTitulo>Resultados de la carga</ResultadoTitulo>
          {resultado.matriculados && resultado.matriculados.length > 0 && (
            <ListaExitosa>
              <SubTitulo>✅ Estudiantes matriculados exitosamente ({resultado.matriculados.length}):</SubTitulo>
              {resultado.matriculados.map((id, index) => (
          ...
          {resultado.errores && resultado.errores.length > 0 && (
            <ListaErrores>
              <SubTituloError>❌ Errores encontrados ({resultado.errores.length}):</SubTituloError>
...
export default MatricularEstudianteCursoCsv;