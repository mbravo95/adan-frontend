...
const CrearCursoCsv = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${urlBase}/cursos/alta-csv`, formData, config);
      ...
      if (totalRegistrados > 0) {
        toast.success(`${totalRegistrados} cursos creados exitosamente`);
      }
      if (totalErrores > 0) {
  ...
  return (
    <Container>
    <ContentWrapper>
      <Title>Crear Cursos desde CSV</Title>
      ...
      >
        {loading ? "Procesando..." : "Cargar Cursos"}
      </UploadButton>
      ...
      {resultado && (
        <ResultadoContainer>
          ...
          {resultado.registrados && resultado.registrados.length > 0 && (
            <ListaExitosa>
              <SubTitulo>✅ Cursos creados exitosamente ({resultado.registrados.length}):</SubTitulo>
              {resultado.registrados.map((curso, index) => (
